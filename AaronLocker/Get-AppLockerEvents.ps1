<#
.SYNOPSIS
Retrieves and sorts event data from AppLocker logs, removes duplicates, and reports as tab-delimited CSV output.
...
Output can be converted to objects with ConvertFrom-Csv for further filtering and processing.
...
.\Get-AppLockerEvents.ps1 | ConvertFrom-Csv -Delimiter "`t" | Where-Object { [datetime]($_.EventTime) -gt "8/20/2017" }
...
.\Get-AppLockerEvents.ps1 -Allowed | ConvertFrom-Csv -Delimiter "`t" | Group-Object PublisherName
